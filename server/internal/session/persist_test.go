...
// the SQLite database, and that HasHero is reconstructed.
...
// TestInMemoryStoreDoesNotPersist confirms NewStore() (no db) writes nothing.
func TestInMemoryStoreDoesNotPersist(t *testing.T) {
	s := NewStore()
	defer s.Close()
	u, _, _ := s.LoginOrRegister("x@y.z", "pw")
...
func TestHeroCustomizationClamped(t *testing.T) {
	...
	// Simulate a legacy record with -1 values already stored.
	h.SkinColor = -1
	h.HairColor = -1
	s1.Save()
	s2 := NewPersistentStore(path)
