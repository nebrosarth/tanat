...
func TestMobFogOfWar(t *testing.T) {
	...
	far := &mobState{id: 3000, mobIdx: 2, mob: mob, x: sx + float32(mobHideRadius) + 5, y: sy, hp: mob.Health}
	near := &mobState{id: 3001, mobIdx: 2, mob: mob, x: sx + 5, y: sy, hp: mob.Health}
	hs.mobs[far.id] = far
	...
	s.updateMobVisibilityLocked(c, far, now)
	s.updateMobVisibilityLocked(c, near, now)
	if far.shown {
...
func TestMobShadeFogRing(t *testing.T) {
	...
	m := &mobState{id: 4000, mobIdx: 2, mob: mob, x: sx + 26, y: sy, hp: mob.Health}
	hs.mobs[m.id] = m
