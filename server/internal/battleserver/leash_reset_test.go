...
// TestMobLeashReAggrosIfPlayerReturns: a mob walking home re-engages (cancelling
// the retreat) if a player steps back inside aggro range before it reaches spawn.
func TestMobLeashReAggrosIfPlayerReturns(t *testing.T) {
	s, c, _, sx, sy := newNavConn(t)
	idx := mobIndexByPrefab(t, "Mob_Skeleton_1H_Melee_01")
	mob := gamedata.Mobs()[idx]
	m := &mobState{
		id: 2201, mobIdx: idx, mob: mob,
		x: sx + 30, y: sy, spawnX: sx + 15, spawnY: sy,
	...
	s.tickMobsLocked(c, 0.2)
	if !m.returning {
		t.Fatal("mob should be returning after leash break")
	}
	// Player runs back next to the mob (within aggro range).
	c.x, c.y, c.snapT = m.x-2, m.y, float32(0)
	s.tickMobsLocked(c, 0.4)
	if m.returning {
		t.Fatal("a player back inside aggro range should cancel the return")
	}
	if !m.aggro {
		t.Fatal("mob should have re-aggroed the returned player")
	}
}
